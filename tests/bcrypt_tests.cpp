...
TEST(bcrypt, Basic) {
  std::vector<uTest> tests{
      ...
      {
          "kzRf4i9CyAQaqjS3r99GUxqRwALdLsXAX7nGXBW2Dz7etAgT7vpDEOTKoSfybSBu"
          "q",
  ...
  for (const auto test : tests) {
    auto receivedHash = bcrypt::generate_hash(test.Password);
    EXPECT_TRUE(bcrypt::compare_hash_and_password(test.Hash, test.Password));
    EXPECT_TRUE(bcrypt::compare_hash_and_password(receivedHash, test.Password));
  }
}